...
function Sidebar() {
  ...
  useSessionStore((s2) => s2.sessions);
  ...
  const [wsSessions, setWsSessions] = reactExports.useState({});
  reactExports.useEffect(() => {
    fetchWorkspaces();
  }, []);
  reactExports.useEffect(() => {
  ...
  const allWorkspaces = reactExports.useCallback(() => {
    const seen = /* @__PURE__ */ new Set();
    const list = [];
    if (cwd) {
      list.push({ path: cwd, name: cwd.split(/[\\/]/).pop() || cwd });
      seen.add(cwd);
    }
    for (const ws of workspaces) {
      const p2 = ws.path || ws.slug || "";
      if (p2 && !seen.has(p2)) {
        list.push({ path: p2, name: p2.split(/[\\/]/).pop() || p2, sessionCount: ws.session_count });
        seen.add(p2);
      }
    }
    return list;
  ...
  const loadWsSessions = async (wsPath) => {
    if (wsSessions[wsPath]) return;
    const prevCwd = useWorkspaceStore.getState().cwd;
    try {
      await switchWorkspace(wsPath);
      const sessions2 = await api.fetchSessions();
      setWsSessions((prev) => ({ ...prev, [wsPath]: sessions2 }));
    } catch {
    }
    if (prevCwd && prevCwd !== wsPath) {
      try {
        await switchWorkspace(prevCwd);
      } catch {
      }
    }
  };
  const toggleExpand = async (wsPath) => {
    const isOpen = expanded[wsPath];
    if (!isOpen) {
      await loadWsSessions(wsPath);
    }
    setExpanded((prev) => ({ ...prev, [wsPath]: !isOpen }));
  };
  ...
  const handleNewSession = async (wsPath) => {
    ...
    await newSession();
    fetchSessions();
    setWsSessions((prev) => ({ ...prev, [wsPath]: [] }));
    loadWsSessions(wsPath);
  };
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        ...
        onDrop: handleDrop,
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-3 py-3 border-b border-[var(--border)]", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "button",
            {
              onClick: handleBrowseNewProject,
              className: `w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg border-2 border-dashed text-sm font-medium transition-all ${dragOver ? "border-[var(--accent)] bg-[var(--accent)]/5 text-[var(--accent)]" : "border-[var(--border)] text-[var(--text3)] hover:border-[var(--accent)] hover:text-[var(--accent)]"}`,
              children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 overflow-y-auto min-h-0", children: [
            wsList.map((ws) => {
              ...
              const sessions2 = isCurrent ? sessions2 : wsSessions[ws.path] || [];
              return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "border-b border-[var(--border)] last:border-b-0", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "div",
                  {
                    className: "flex items-center gap-1.5 px-2 py-1.5 hover:bg-[var(--surface2)] cursor-pointer transition-colors group",
                    ...
                    children: [
                      ...
                      isCurrent && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "w-1.5 h-1.5 rounded-full bg-[var(--green)] shrink-0", title: "Active" }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                        "button",
                        {
                          onClick: (e) => {
                ...
                isOpen && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "pb-1", children: sessions2.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-8 py-2 text-[10px] text-[var(--text3)]", children: "No sessions" }) : sessions2.map((s2) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "div",
                  {
                    onClick: () => {
                      if (!isCurrent) handleSwitchWorkspace(ws.path).then(() => handleLoadSession(s2.session_id));
                      else handleLoadSession(s2.session_id);
                    },
            ...
            wsList.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-4 py-8 text-center text-[11px] text-[var(--text3)]", children: [
              "No projects yet.",
