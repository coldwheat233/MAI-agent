...
function InputTextarea({ value, onChange, onSubmit, disabled }) {
  ...
  reactExports.useEffect(() => {
    ...
    const newHeight = Math.min(el2.scrollHeight, maxHeight);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    "textarea",
    {
      ...
      disabled,
      rows: 1,
      className: "flex-1 bg-[var(--bg)] border border-[var(--border)] rounded-xl text-sm text-[var(--text)] px-4 py-2.5 resize-none outline-none focus:border-[var(--accent)] transition-colors placeholder:text-[var(--text3)] disabled:opacity-50 font-sans"
    }
  );
}
